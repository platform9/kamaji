...
func NewCmd(scheme *runtime.Scheme) *cobra.Command {
	...
	cmd.Flags().StringVar(&migrateJobImage, "migrate-image", fmt.Sprintf("%s/%s/kamaji:%s", internal.ContainerRepository, internal.ContainerOrganization, internal.GitTag), "Specify the container image to launch when a TenantControlPlane is migrated to a new datastore.")
